...
int main(int argc, char *argv[]) {
  ...
  if (argc != 6) {
    printf("Usage: <input_image_path> <filter_radius> <filter_iteration_num> <median_filter_output_path> <side_window_median_filter_output_path>\n");
    return 0;
  }
  ...
  std::string median_filter_out_path             = std::string(argv[4]);
  std::string side_window_median_filter_out_path = std::string(argv[5]);
  ...
  median_filter.init(height, width, radius);

  for (int i = 0; i < channel; ++i) {
    median_filter.fastMedianFilter(channels[i].data(), radius, height, width, results[i].data());
  }
  for (int i = 0; i < iteration; ++i) {
    for (int c = 0; c < channel; ++c) {
      median_filter.fastMedianFilter(results[c].data(), radius, height, width, results[c].data());
    }
  }
  ...
  stbi_write_jpg(median_filter_out_path.c_str(), width, height, channel, result.data(), 100);

  SideWindowMedianFilter side_window_median_filter;
  side_window_median_filter.init(height, width, radius);

  for (int i = 0; i < channel; ++i) {
    side_window_median_filter.fastSideWindowMedianFilter(channels[i].data(), radius, height, width, results[i].data());
  }
  for (int i = 0; i < iteration; ++i) {
    for (int c = 0; c < channel; ++c) {
      side_window_median_filter.fastSideWindowMedianFilter(results[c].data(), radius, height, width, results[c].data());
    }
  }

  mergeChannels(results, height, width, channel, &result[0]);
  stbi_write_jpg(side_window_median_filter_out_path.c_str(), width, height, channel, result.data(), 100);
